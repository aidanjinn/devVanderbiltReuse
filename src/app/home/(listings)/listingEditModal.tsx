...
export default function ListingEditModal({
  ...
  async function handleSubmit() {
    ...
    try {
      let res = await editListing(id, listing.id, {
        title,
        description,
        price,
        location,
        status,
        category,
      }); // Include category in the edit payload
      if (res) {
        refetchListings();
        close();
      }
    ...
    <>
      <Modal opened={open} onClose={close} withCloseButton={false}>
        ...
        <TextInput
          ...
          label="Title"
          placeholder="Mini Fridge,  Math 1501 Textbook,  etc."
        />
        <TextInput
          value={description}
        ...
        <Select
          value={status}
          onChange={(e) => setStatus(e ?? "available")}
          label="Status"
          data={["available", "sold"]}
        />
        ...
        <div className="flex w-full gap-x-3 justify-between mt-3">
          <Button variant="filled" color="red" onClick={handleDeleteListing}>
            Delete
          </Button>
          <Group gap={8}>
